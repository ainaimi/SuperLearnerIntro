...
beta<- c(2,beta)   #*************************************** LB: could change the intercept to make outcome more common
...
sl.lib <- c("SL.bayesglm","SL.randomForest")
...
m2<-lapply(1:folds,function(ii) randomForest(formula=as.factor(y)~.,data=do.call(rbind,splt[-ii])))
...
p2<-lapply(1:folds,function(ii) predict(m2[[ii]],newdata=data.frame(rbindlist(splt[ii])),type="prob")[,2])

# update dataframe 'splt' so that column1 is the observed outcome (y)
#   column2 is the CV-predicted probability of the outcome from earth
#   column3 is the CV-predicted probability of the outcome from rpart
for(i in 1:folds){
...
risk2<-lapply(1:folds,function(ii) 1-AUC(predictions=splt[[ii]][,3], labels=splt[[ii]][,1]))		# CV-risk for rpart
...
xtable(a)
# compare with the package output  ********************* check
fitY; a

#----------------------------
## 4: estimate SL weights using the optim() function to minimize (1-AUC)
X<-data.frame(do.call(rbind,splt),row.names=NULL);  names(X)<-c("y","bayesglm","rF")
head(X)

bounds = c(0, Inf)
SL.r<-function(A, y, par){
  A<-as.matrix(A)
  names(par)<-c("bayesglm","rF")
...
#************ simplifying the filename ----- maybe add the level-0 fits? 
pdf(file="figure2.pdf",height=4,width=5)
cols <- c("SuperLearner Package"="red","Manual SuperLearner"="blue")
ggplot() +
  geom_step(data=C, aes(1-spec,sens),color="blue",size=.75) +
  geom_step(data=C2, aes(1-spec,sens),color="red",linetype=2,size=.5) +
